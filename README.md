...
이 저장소에는 서로 독립적인 두 구현이 공존합니다.

| 버전 | 파일 | 특징 |
|---|---|---|
| 스크립트 버전 | `notebooks/day2_security_rag.py` | PDF 자동 선별, 결정적 ID 멱등 저장, 로컬 해시 임베딩 폴백 |
| 노트북 버전 | `notebooks/day2_hybrid_rag.ipynb` | 하이브리드 검색(BM25+RRF), 카테고리 필터, 이식용 셀 제공 |

## 노트북 버전: `notebooks/day2_hybrid_rag.ipynb`
...
PDF 경로는 노트북 상단 `PDF_DIR` 변수 하나로 관리합니다 (기본값 `datasets/보안 pdf모음/`, 팀원 환경 예: `../datasets/보안 취약점 PDF`).
